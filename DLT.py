...
def DLTcalib(nd, xyz, uv):
    ...
    if (nd == 2 and xyz.shape[1] != 2) or (nd == 3 and xyz.shape[1] != 3):
        raise ValueError('Incorrect number of coordinates ni(%d) for %dD DLT (it should be %d).' %(xyz.shape[1],nd,nd))
...
def accuracy_assessment(L, XYZ):

    xyz_checkpoints =  N.asarray(XYZ)

    L_check = N.asarray(L)

    #print(xyz_checkpoints, L_check)
    #print(xyz_checkpoints[0][0])
    x_image_point_num = L_check[0] * xyz_checkpoints[0][0]+ L_check[1] * xyz_checkpoints[0][1] + L_check[2] * xyz_checkpoints[0][2] + L_check[3]
    
    y_image_point_num = L_check[4] * xyz_checkpoints[0][0]+ L_check[5] * xyz_checkpoints[0][1] + L_check[6] * xyz_checkpoints[0][2] + L_check[7]

    image_point_den = L_check[8] * xyz_checkpoints[0][0]+ L_check[9] * xyz_checkpoints[0][1] + L_check[10] * xyz_checkpoints[0][2] + L_check[11]

    
    x_image_point = x_image_point_num/image_point_den
    y_image_point = y_image_point_num/image_point_den

    print(x_image_point, y_image_point)

def createOrthoImage(offnadir_image,DLTparams,output_file):

    image = numpy.asarray()

    pixelValue = array[1,2]
...
if __name__ == '__main__':
    ...
    nd = 3
    xyz = [[491251.5683, 5459281.2630, 107.9400], [490705.4007, 5459589.8930, 58.9120], [491489.6055, 5459014.2700, 127.0180], [490339.2088, 5458684.2230, 96.8950], [490221.6380, 5458881.9720, 82.1310], [490256.8843, 5459268.5820, 75.8610], [490586.7815, 5459605.7680, 55.3180], [490902.4005, 5458567.2510, 46.7930], [489938.6693, 5458785.1190, 53.5340]]

    uv1 = [[6292, 5285],[5809, 5760],[5833, 5933],[6277, 6728],[6658, 7065],[7293, 6202], [7126, 5409],[6844, 5245], [6982, 4838]]


    XYZ = [[491251.5683, 5459281.2630, 107.9400], [490705.4007, 5459589.8930, 58.9120], [491489.6055, 5459014.2700, 127.0180], [490339.2088, 5458684.2230, 96.8950], [490221.6380, 5458881.9720, 82.1310], [490256.8843, 5459268.5820, 75.8610], [490586.7815, 5459605.7680, 55.3180], [490902.4005, 5458567.2510, 46.7930], [489938.6693, 5458785.1190, 53.5340]]

    #XYZ =[[490806.3594, 5459298.249, 35.092],[491068.3346, 5458957.454, 33.03],[490457.8185, 5458632.605, 39.907],[491169.4798, 5458623.948, 27.863],[491074.7131, 5458023.455, 11.833],[489957.5598, 5459672.76, 14.453]]

    L, err = DLTcalib(nd, xyz, uv1)

    #print(L)

    #offnadir_image = rasterio.open("C:/Users/sarat/OneDrive/Documents/data/Vancouver_39372/20210925_191426_SN18_L3_SR_MS.tif")


    accuracy_assessment(L, XYZ)
